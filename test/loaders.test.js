...
import { Message, initializeAsyncSources as ias, putter } from '../src'

let _networkError = new Error('some network error!')
function fetchSync(i, cb) { return cb(++i) }
function fetchAsync(i, cb) { return setTimeout(() => cb(++i)) }
function fetchAsyncErr(i, cb) { return setTimeout(() => cb(_networkError)) }
