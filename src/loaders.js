...
import { putter, multTapper } from './extensions'

const defaultOptions = { abandonTimeout: 4000 }
const _abandonErrorMsg = 'async task abandoned due to a timeout'
...
function* serialLoader(fetch, success, error, source, {abandonTimeout}) {
  let msg, [block, unblock] = createMults(source), startTimer = () => timeout(abandonTimeout)
  let abandoned = meta => error(Message.fail(_abandonErrorMsg, meta))
...
const initializeAsyncSources = (loaderKey = 'serial', options = defaultOptions) =>
  (fetch, success, error, ...triggers) =>
    triggers.forEach(c => go(loaders[loaderKey], [fetch, success, error, c, options]))

export default initializeAsyncSources